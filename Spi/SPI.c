...
static uint8_t  m_tx_buf[SPIS_BUFFER]={0};
...
void spis_process(void)
{
	switch(m_spistatus)
	{
		case SPISTATUS_INIT:
		{
			...
			index = 0;
			while(index<SPIS_BUFFER)
			{
				m_tx_buf[index] = index;
				++index;
			}
		...
		{
			/*recved data process*/
			uint8_t const_data[SPIS_BUFFER];
			uint8_t const_index=0;
			while(const_index<SPIS_BUFFER)
			{
				const_data[const_index] = const_index;
				++const_index;
			}
			
			if(m_real_recved==SPIS_BUFFER && m_real_sended==SPIS_BUFFER)
			{
				nrf_gpio_pin_toggle(21);
			}
			if(memcmp(const_data, m_rx_buf, SPIS_BUFFER)==0)
			{
